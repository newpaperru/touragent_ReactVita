...
import { useHostelData } from "./customHooks/useHostelData";
import { useFixedHeader } from "./customHooks/useFixedHeader";

export const HostelRoom = () => {
    const { hostelId } = useParams();
    const { hostel, error } = useHostelData(hostelId);
    ...
    return (
        <>
            ...
            <div ref={headerRef} style={{ height: isHeaderFixed ? 0 : 'auto' }}>
                {!isHeaderFixed && (
                    <HeaderHostelRoom header_data={hostel.hostelTitle} />
                )}
            </div>
            {isHeaderFixed && (
                <HeaderHostelRoom 
                    header_data={hostel.hostelTitle} 
                    isFixed 
                />
            )}
            <div className={styles.hostel_room}>
                <MainInfoHostelRoom hostel={hostel} />
            </div>
            <Footer />
        </>
    );
